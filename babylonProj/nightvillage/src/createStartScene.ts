...
function createTerrain(scene: Scene) {
  ...
  function createHouses(scene: Scene, style: number) {
    // Start by locating one of the two house types then add others
    
    if (style == 1) {
      // Show 1 small house
      const house = createHouse(scene, 1);
      if (house) createHelicopterAboveHouse(scene, house);  // Add helicopter above the house, only if house is not null
    }
    if (style == 2) {
      // Show 1 large house
      const house = createHouse(scene, 2);
      if (house) createHelicopterAboveHouse(scene, house);  // Add helicopter above the house, only if house is not null
    }
    if (style == 3) {
      // Show estate with multiple houses
      ...
      // Add helicopters above the first two houses
      if (houses[0]) createHelicopterAboveHouse(scene, houses[0]!);
      if (houses[1]) createHelicopterAboveHouse(scene, houses[1]!);
  
      // Next houses are cloned instances of the first two
      ...
      for (let i = 0; i < places.length; i++) {
        ...
        // Add helicopter above each cloned house
        createHelicopterAboveHouse(scene, ihouses[i] as unknown as Mesh);
      }
    }
  ...
  function createHelicopterAboveHouse(scene: Scene, house: Mesh) {
    // Create the sprite manager for the helicopter
    const helicopterManager = new SpriteManager(
      "helicopterManager",
      "./assets/sprites/helicopter1.png",
      2000, // Number of sprites
      { width: 512, height: 512 },
      scene
    );
  
    // Create the helicopter sprite above house
    const helicopter = new Sprite("helicopter", helicopterManager);
  
    // Position the helicopter slightly above the house
    helicopter.position = new Vector3(house.position.x, house.position.y + 3.5, house.position.z);
  
    // Return the helicopter sprite
    return helicopter;
  }
  ...
  function createTrees(scene: Scene) {
    const spriteManagerTrees = new SpriteManager(
      "treesManager",
      "./assets/sprites/burningtree2.png",
      2000,
